...
auto TrieStore::Get(std::string_view key) -> std::optional<ValueGuard<T>> {
  ...
  TrieStore::root_lock_.lock();
  auto root = TrieStore::root_;
  TrieStore::root_lock_.unlock();
  auto value = root.Get<T>(key);
  return value ? std::optional<ValueGuard<T>>(ValueGuard<T>(root, *value)) : std::nullopt;
