...
template <class T>
auto Trie::PutInternal(std::unique_ptr<TrieNode> new_node, std::string_view key, T value) const
    -> std::shared_ptr<const TrieNode> {
  // auto new_node = pre->Clone();
  auto it = new_node->children_.find(key[0]);
  if (key.size() == 1) {
    if (it == new_node->children_.end()) {
      new_node->children_.insert(
          {key[0], std::make_shared<TrieNodeWithValue<T>>(std::make_shared<T>(std::move(value)))});
      return std::shared_ptr<TrieNode>(std::move(new_node));
    }
    auto childnode = it->second;
    auto new_node_with_value =
        std::make_shared<TrieNodeWithValue<T>>(childnode->children_, std::make_shared<T>(std::move(value)));
